...
class Wsspg_Payment_Gateway extends WC_Payment_Gateway_CC {
	...
	public function process_payment( $order_id ) {
		...
		if( ! isset( $_POST['wsspg-data'] ) ) {
            ...
            $customer = Wsspg_Customer::load( $order->get_user_id() );
			...
			if( ! isset( $customer->stripe ) ) {
				$customer->params = array(
					...
					"metadata" => array(
						...
						"telephone"  => $order->data['billing']['phone'],
					),
				);
			...
			foreach( $cart as $cart_item_key => $cart_item ) {
				...
				if( $item->get_type() === 'wsspg_subscription' ) {
					...
					global $wpdb;
					$upd = array('sub_id' => $subscription->id);
					$whereis = array('user_id' => $this_user_id);
					$wpdb->update('wp_mixpro_meels', $upd, $whereis);
